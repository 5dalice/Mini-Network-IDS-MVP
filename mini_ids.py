...
from ids.reporter import filter_alerts_by_severity, print_report, save_json_report
...
def analyze(pcap_path: str, rules_dir: str) -> list[dict]:
    ...
    domain_blocklist = load_blocklist(rules_path / "malicious_domains.txt")
    ip_blocklist = load_blocklist(rules_path / "malicious_ips.txt")

    alerts: list[dict] = []
    alerts.extend(detect_port_scan(packets))
    alerts.extend(detect_syn_flood(packets))
    alerts.extend(detect_malicious_dns(packets, domain_blocklist))
    alerts.extend(detect_large_packets(packets))
    alerts.extend(detect_malicious_ips(packets, ip_blocklist))
    return alerts
...
def main() -> None:
    ...
    alerts = analyze(args.pcap, args.rules)

    if args.severity:
        alerts = filter_alerts_by_severity(alerts, args.severity)

    print_report(alerts, show_summary=args.show_summary)
    save_json_report(alerts, args.output)
